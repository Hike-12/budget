...
export default function CalendarPage() {
  ...
  const { daysInMonth, startDay, monthName, year, currentMonthIndex } =
    useMemo(() => {
  ...
  const prevMonth = () =>
    setCurrentDate(new Date(year, currentMonthIndex - 1, 1));
  const nextMonth = () =>
    setCurrentDate(new Date(year, currentMonthIndex + 1, 1));
  const goToday = () => {
    const today = new Date();
  ...
  const selectedDayData = useMemo(() => {
    ...
    return (
      budgetMap.get(selectedDate.getTime()) || {
        income: 0,
        expense: 0,
        items: [],
      }
          ...
          <div className="p-6">
            <div className="grid grid-cols-7 gap-1 mb-4">
              {WEEKDAYS.map((day) => (
                <div
                  key={day}
                  className="text-center text-[11px] font-bold text-secondary/40 uppercase tracking-widest pb-2"
                >
                  {day}
                </div>
              ))}
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-base font-semibold z-10 ${isSelected ? "text-primary" : isToday ? "text-accent font-bold" : "text-accent/80 group-hover:text-accent"}`}
                    >
                      {dayStr}
                    </span>
              ...
              {isLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3 pt-2">
                    <p className="text-[10px] text-secondary/50 uppercase tracking-widest font-bold border-b border-white/10 pb-2">
                      Transactions
                    </p>
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0 pr-3">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-semibold text-accent/90 group-hover:text-accent truncate transition-colors">
                                {item.title}
                              </p>
                ...
                <div className="text-center py-16 px-4 border border-dashed border-white/10 rounded-2xl bg-white/5">
                  ...
                  <p className="text-xs text-secondary/50 mt-1 max-w-[200px] mx-auto">
                    There are no transactions recorded for this specific date.
                  </p>
                </div>
              )}
            </div>
